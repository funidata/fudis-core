...
- Navigate to the release branch you want to make a new version update from. Release branches should be named `release-{major.minor.patch}`
- Create new branch from the release branch
...
- Check correct release checkbox:
  ...
  > Set as pre-release if it is e.g. release candidate

  > Set as latest if it is the newest release

  > Uncheck bot checkboxes if it is e.g. patch for previously released major
